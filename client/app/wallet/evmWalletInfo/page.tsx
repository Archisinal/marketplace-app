...
import { METHOD_MAP } from '@/features/wallet-connect/methods';
import { Button, Input, message, Select } from 'antd';
// Deprecated need to be converted to @metamask/eth-sig-util
import {
...
if (chain81Index) {
  chainList[chain81Index] = {
    ...
    shortName: 'SBY',
    explorers: [
      { name: 'subscan', url: 'https://shibuya.subscan.io', standard: 'none' },
    ],
  };
}
    ...
    getInput: (
      ...
      return {
        ...
        message: {
          ...
          from: {
            name: 'Cow',
            wallets: [from, '0xDeaDbeefdEAdbeefdEadbEEFdeadbeEFdEaDbeeF'],
          },
          to: [
            {
              name: 'Alice',
...
function EvmWalletInfo(): React.ReactElement {
  ...
  const [warningNetwork, setWarningNetwork] = useState<string | undefined>(
    undefined,
  );

  const [lastestBlock, setLastestBlock] = useState<number | undefined>(
    undefined,
  );
  ...
  const generateRequestButton = useCallback(
    (
      label: string,
      args: RequestArguments,
      callback?: (rs: Maybe<unknown>) => void,
      disabled?: boolean,
    ) => (
      <Button
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const w3 =
        //   @ts-ignore
        wallet?.extension && new Web3(wallet.extension as AbstractProvider);
      ...
      if (_chainId) {
        ...
        const _accounts = await wallet?.request<string[]>({
          method: 'eth_accounts',
        });
        ...
        setWarningNetwork(
          'Please select at least one button below to switch to EVM network',
        );
      }
  ...
  const _onChangeSignMethod = useCallback((value: string) => {
    setSignMethod(value);
    setSignature('');
    setSignatureValidation('');
  }, []);

  const handlePermissionsRs = useCallback((response: Maybe<unknown>) => {
    // @ts-ignore
  ...
  const sendTransaction = useCallback(() => {
    makeRequest(
        ...
        params: [
          {
            ...
            value:
              ...
              (
                transactionAmount *
                10 ** (network?.nativeCurrency.decimals || 18)
              ).toString(16),
      ...
      (transactionHash) => {
        if (
          network?.explorers &&
          network?.explorers.length &&
          transactionHash
        ) {
          const explorer = network?.explorers[0]?.url;

          setTransactionLink(
            explorer && explorer + '/tx/' + (transactionHash as string),
          );
        } else {
  ...
  const signData = useCallback(() => {
    ...
    if (signMethod === 'ethSign') {
      ...
      args.params = [
        from,
        JSON.stringify(
          SIGN_METHODS.signTypedDatav3.getInput(
            signMessage,
            chainId || 0,
            from,
          ),
      ...
      args.params = [
        from,
        JSON.stringify(
          SIGN_METHODS.signTypedDatav4.getInput(
            signMessage,
            chainId || 0,
            from,
          ),
  ...
  const verifySignature = useCallback(() => {
    ...
    if (signMethod === 'ethSign') {
      ...
      recoveredAddress = recoverTypedSignature({
        data: SIGN_METHODS.signTypedDatav3.getInput(
          signMessage,
          chainId || 0,
          from,
        ),
      ...
      recoveredAddress = recoverTypedSignature_v4({
        data: SIGN_METHODS.signTypedDatav4.getInput(
          signMessage,
          chainId || 0,
          from,
        ),
      ...
      <div className={'evm-wallet-info-page'}>
        <div className="evm-wallet-info-page__section">
          ...
          <div>
            Network:{' '}
            {network && (
              <span className="account-item__content">
          ...
          <div>
            Status:{' '}
            <span className="account-item__content">
              {wallet?.extension?.isConnected() && chainId
                ? 'Connected'
                : 'Disconnected'}
            </span>
          </div>
          <div>
            Current Address:{' '}
            <span className="account-item__content font-mono">
              {accounts.join(', ')}
            </span>
          </div>
          <div>
        ...
        <div className="evm-wallet-info-page__section">
          ...
          <div className="evm-wallet-info__button_group">
            {generateRequestButton(
              'Get Permissions',
              METHOD_MAP.getPermissions,
              handlePermissionsRs,
            )}
            {generateRequestButton(
              'Request Permissions',
              METHOD_MAP.requestPermissions,
              handlePermissionsRs,
            )}
          </div>
        ...
        <div className="evm-wallet-info-page__section">
          ...
          {warningNetwork && (
            <div className="warning-text">{warningNetwork}</div>
          )}
          <div className="evm-wallet-info__button_group">
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Moonbeam Network',
                METHOD_MAP.addMoonbeamNetwork,
                undefined,
                chainId === 1284,
              )}
              {generateRequestButton(
                'Switch to Moonbeam',
                METHOD_MAP.switchToMoonbeamNetwork,
                undefined,
                chainId === 1284,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Moonriver Network',
                METHOD_MAP.addMoonriverNetwork,
                undefined,
                chainId === 1285,
              )}
              {generateRequestButton(
                'Switch to Moonriver',
                METHOD_MAP.switchToMoonriverNetwork,
                undefined,
                chainId === 1285,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Moonbase Network',
                METHOD_MAP.addMoonbaseAlphaNetwork,
                undefined,
                chainId === 1287,
              )}
              {generateRequestButton(
                'Switch to Moonbase',
                METHOD_MAP.switchToMoonbaseAlphaNetwork,
                undefined,
                chainId === 1287,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Astar Network',
                METHOD_MAP.addAstarNetwork,
                undefined,
                chainId === 592,
              )}
              {generateRequestButton(
                'Switch to Astar',
                METHOD_MAP.switchToAstarNetwork,
                undefined,
                chainId === 592,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Shiden Network',
                METHOD_MAP.addShidenNetwork,
                undefined,
                chainId === 336,
              )}
              {generateRequestButton(
                'Switch to Shiden',
                METHOD_MAP.switchToShidenNetwork,
                undefined,
                chainId === 336,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Shibuya Network',
                METHOD_MAP.addShibuyaNetwork,
                undefined,
                chainId === 81,
              )}
              {generateRequestButton(
                'Switch to Shibuya',
                METHOD_MAP.switchToShibuyaNetwork,
                undefined,
                chainId === 81,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Mumbai Network',
                METHOD_MAP.addMumbaiNetwork,
                undefined,
                chainId === 80001,
              )}
              {generateRequestButton(
                'Switch to Mumbai',
                METHOD_MAP.switchToMumbaiNetwork,
                undefined,
                chainId === 80001,
              )}
            </div>
            <div className="evm-wallet-info__button_row">
              {generateRequestButton(
                'Add Boba Testnet',
                METHOD_MAP.addBobaTestnet,
                undefined,
                chainId === 1297,
              )}
              {generateRequestButton(
                'Switch to Boba Testnet',
                METHOD_MAP.switchToBobaTestnet,
                undefined,
                chainId === 1297,
              )}
            </div>
        ...
        <div className="evm-wallet-info-page__section">
          ...
          <div className="evm-wallet-transaction_row">
            ...
            <Input className="code" readOnly type="text" value={accounts[0]} />
          </div>
          <div className="evm-wallet-transaction_row">
          ...
          <div>
            ...
            {transactionLink && (
              <div>
                ...
                <a href={transactionLink} rel="noreferrer" target="_blank">
                  {' '}
                  here
                </a>
              </div>
        ...
        <div className="evm-wallet-info-page__section">
          ...
          <div className="evm-wallet-transaction_row">
            ...
            <Select defaultValue={signMethod} onChange={_onChangeSignMethod}>
              {Object.entries(SIGN_METHODS).map(([k, v]) => (
        ...
        <div className="evm-wallet-info-page__section">
          <div className="evm-wallet-info-page__text">
            Connect provider with web3.js
          </div>
          <div>
            Latest block:{' '}
            <span className="account-item__content">{lastestBlock}</span>
          </div>
        </div>
      </div>
    </div>
