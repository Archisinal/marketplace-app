...
export default class EventsClass {
	...
	public subscribeOnUserDataSetEvent(callback : (event : EventTypes.UserDataSet) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('UserDataSet', EVENT_DATA_TYPE_DESCRIPTIONS)) as EventTypes.UserDataSet);
		};

		return this.__subscribeOnEvent(callbackWrapper, (eventName : string) => eventName == 'UserDataSet');
	}
